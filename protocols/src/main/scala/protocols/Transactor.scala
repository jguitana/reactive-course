...
object Transactor {
  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] = Behaviors.receive {
    case (ctx, Begin(replyTo)) =>
      ...
      ctx.schedule(sessionTimeout, sessionRef, Rollback[T]())
      replyTo ! sessionRef
  ...
  private def inSession[T](rollbackValue: T,
                           ...
                           sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] = Behaviors.setup { ctx =>
    Behaviors.receiveMessage[PrivateCommand[T]] {
      case Committed(session, t) =>
        if (sessionRef == session) idle(t, sessionTimeout)
        else Behaviors.same
  ...
  private def sessionHandler[T](currentValue: T,
                                ...
                                done: Set[Long]): Behavior[Session[T]] = Behaviors.setup { ctx =>
    Behaviors.receiveMessage {
      case x: Extract[T, _] =>
        x.replyTo ! x.f(currentValue)
        Behaviors.same
      ...
      case x: Modify[T, _] =>
        if (done(x.id)) {
          ...
          val modified = x.f(currentValue)
          x.replyTo ! x.reply
          sessionHandler(modified, commit, done + x.id)
        }
