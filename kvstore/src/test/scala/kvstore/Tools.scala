 ...
 */
trait Tools { this: FunSuiteLike with Matchers =>
  ...
  class Session(val probe: TestProbe, val replica: ActorRef) {
    ...
    def set(key: String, value: String, id: Long): Unit = {
      probe.send(replica, Insert(key, value, id))
    }
    ...
    def remove(key: String, id: Long) = {
      probe.send(replica, Remove(key, id))
    }
    ...
    def get(key: String): Option[String] = {
      val s = nextSeq
      probe.send(replica, Get(key, s))
      probe.expectMsgType[GetResult](5.seconds).valueOption
    }
