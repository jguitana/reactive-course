...
object Server {
  ...
  val eventParserFlow: Flow[ByteString, Event, NotUsed] =
    Flow[ByteString].via(reframedFlow).map(Event.parse)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    ...
    {
      case event if event.sequenceNr > current =>
        val (send, keep) = (event +: buffer).sortBy(_.sequenceNr).span { e =>
          if (e.sequenceNr == current + 1) {
            current += 1
            true
          } else false
        }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
...

    {
      case e@Event.Follow(_, fromUserId, toUserId) =>
        followers = followers.get(fromUserId)
          .fold(followers + (fromUserId -> Set(toUserId)))(f =>
            followers.updated(fromUserId, f + toUserId))
        List((e, followers))

      case e@Event.Unfollow(_, fromUserId, toUserId) =>
        followers = followers.get(fromUserId)
          .fold(followers + (fromUserId-> Set.empty[Int]))(f =>
            followers.updated(fromUserId, f - toUserId))
        List((e, followers))
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = {
    ...
    event match {
      case x: Event.Follow =>
        x.toUserId == userId
      case x: Event.Broadcast =>
        true
      case x: Event.PrivateMsg =>
        x.toUserId == userId
      case x: Event.StatusUpdate =>
        followers.get(userId).exists(_(x.fromUserId))
      case _ =>
        false
    }
  }
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut.filter(isNotified(userId)(_)).map(_._1.render)
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] = {
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue(clientIdPromise.completeWith).mapMaterializedValue(_ => NotUsed)
